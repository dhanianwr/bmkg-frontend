...
export function CuacaBesok() {
  ...
  const [nameBO, setNameBO] = useState(new Date());
  const [fotoBO, setFotoBO] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const recordsPerPage = 12;
  const lastIndex = currentPage * recordsPerPage;
  const firstIndex = lastIndex - recordsPerPage;
  const records = imagesBO.slice(firstIndex, lastIndex);
  const npage = Math.ceil(imagesBO.length / recordsPerPage);
  ...
  const getImageBO = async () => {
    try {
      const response = await axios.get("http://localhost:5000/cuaca-besok");
  ...
  const createCuacaHariIni = async (e) => {
    let fixDate = nameBO.toISOString();
    e.preventDefault();
    const formData = new FormData();
    formData.append("name", fixDate);
    formData.append("foto", fotoBO);
    try {
      await axios.post("http://localhost:5000/cuaca-besok", formData, {
        headers: {
          "Content-type": "multipart/form-data",
        },
      });
      refreshPage();
      alert("Data Prakiraan Cuaca Besok Berhasil Ditambah");
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const deleteImageBO = async (buletinId) => {
    await axios.delete(`http://localhost:5000/cuaca-besok/${buletinId}`);
    mutate("cuaca-besok");
    alert("Data Prakiraan Cuaca Besok Berhasil Dihapus");
    refreshPage();
  };
      ...
      <div className="bg-blueGray-100 relative mx-auto my-5 flex max-w-full flex-col break-words rounded-md border-t-[6px] border-t-cyan-500 shadow-lg">
        ...
        <div className="flex-auto bg-[#ffffffe5] lg:px-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 my-5 px-6 h-full w-full">
            {records.map((cuacaBO, index) => {
               var nama = new Date(cuacaBO.name).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
              ...
              var TanggalBikin = new Date(cuacaBO.createdAt).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
              ...
              var TanggalUpdate = new Date(cuacaBO.updatedAt).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
              ...
              return (
                ...
                >
                  ...
                  <CardFooter className="flex flex-row gap-2 self-center pt-0">
                    <a
                      href={`/dashboard/editCuacaBesok/${cuacaBO.id}`}
                    >
