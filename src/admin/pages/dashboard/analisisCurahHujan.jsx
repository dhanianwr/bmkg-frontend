...
export function AnalisisCurahHujan() {
  ...
  const [nameACH, setNameACH] = useState(new Date());
  const [fotoACH, setFotoACH] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const recordsPerPage = 12;
  const lastIndex = currentPage * recordsPerPage;
  const firstIndex = lastIndex - recordsPerPage;
  const records = imagesACH.slice(firstIndex, lastIndex);
  const npage = Math.ceil(imagesACH.length / recordsPerPage);
  ...
  const getImageACH = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/analisis-curah-hujan"
      );
  ...
  const createCuacaHariIni = async (e) => {
    let fixDate = nameACH.toISOString();
    e.preventDefault();
    const formData = new FormData();
    formData.append("name", fixDate);
    formData.append("foto", fotoACH);
    try {
      await axios.post("http://localhost:5000/analisis-curah-hujan", formData, {
        headers: {
          "Content-type": "multipart/form-data",
        },
      });
      refreshPage();
      alert("Data Analisis Curah Hujan Berhasil Ditambah");
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const deleteImageACH = async (buletinId) => {
    await axios.delete(
      `http://localhost:5000/analisis-curah-hujan/${buletinId}`
    );
    mutate("analisis-curah-hujan");
  ...
  return (
    <div>
      <div className="bg-blueGray-100 relative mx-auto my-5 flex max-w-full flex-col break-words rounded-md border-t-[6px] border-t-cyan-500 shadow-lg">
        <div className="flex-auto border-b-gray-300 bg-white p-2">
          <div className="flex justify-between text-center">
            <h6 className="text-blueGray-700 pl-5 text-xl">
              Input Analisis Curah Hujan
            </h6>
          </div>
        </div>
      ...
      <div className="bg-blueGray-100 relative mx-auto my-5 flex max-w-full flex-col break-words rounded-md border-t-[6px] border-t-cyan-500 shadow-lg">
        ...
        <div className="flex-auto bg-[#ffffffe5] lg:px-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 my-5 px-6 h-full w-full">
            {records.map((cuacaACH, index) => {
               var nama = new Date(cuacaACH.name).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
              ...
              var TanggalBikin = new Date(cuacaACH.createdAt).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
              ...
              var TanggalUpdate = new Date(cuacaACH.updatedAt).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
