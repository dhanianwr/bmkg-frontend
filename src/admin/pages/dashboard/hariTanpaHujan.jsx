...
export function HariTanpaHujan() {
  ...
  const [nameHTH, setNameHTH] = useState(new Date());
  const [fotoHTH, setFotoHTH] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const recordsPerPage = 20;
  const lastIndex = currentPage * recordsPerPage;
  const firstIndex = lastIndex - recordsPerPage;
  const records = imagesHTH.slice(firstIndex, lastIndex);
  const npage = Math.ceil(imagesHTH.length / recordsPerPage);
  ...
  const getImageHTH = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/hari-tanpa-hujan"
      );
  ...
  const createCuacaHariIni = async (e) => {
    let fixDate = nameHTH.toISOString();
    e.preventDefault();
    ...
    try {
      ...
      refreshPage();
      alert("Data Prakiraan Hari Tanda Hujan Berhasil Ditambah");
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const deleteImageHTH = async (buletinId) => {
    await axios.delete(`http://localhost:5000/hari-tanpa-hujan/${buletinId}`);
    mutate("hari-tanpa-hujan");
      ...
      <div className="bg-blueGray-100 relative mx-auto my-5 flex max-w-full flex-col break-words rounded-md border-t-[6px] border-t-cyan-500 shadow-lg">
        ...
        <div className="flex-auto bg-[#ffffffe5] lg:px-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 my-5 px-6 h-full w-full">
            {records.map((cuacaHTH, index) => {
               var nama = new Date(cuacaHTH.name).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
              ...
              var TanggalBikin = new Date(cuacaHTH.createdAt).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
              ...
              var TanggalUpdate = new Date(cuacaHTH.updatedAt).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
              ...
              return (
                ...
                >
                  ...
                  <CardFooter className="flex flex-row gap-2 self-center pt-0">
                    <a
                      href={`/dashboard/editHariTanpaHujan/${cuacaHTH.id}`}
                    >
