...
export function AnalisisSifatHujan() {
  ...
  const [nameASH, setNameASH] = useState(new Date());
  const [fotoASH, setFotoASH] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const recordsPerPage = 8;
  const lastIndex = currentPage * recordsPerPage;
  const firstIndex = lastIndex - recordsPerPage;
  const records = imagesASH.slice(firstIndex, lastIndex);
  const npage = Math.ceil(imagesASH.length / recordsPerPage);
  ...
  const createCuacaHariIni = async (e) => {
    let fixDate = nameASH.toISOString();
    e.preventDefault();
    ...
    try {
      ...
      refreshPage();
      alert("Data Analisis Sifat Hujan Berhasil Ditambah");
    } catch (error) {
      console.log(error);
    }
  };
      ...
      <div className="bg-blueGray-100 relative mx-auto my-5 flex max-w-full flex-col break-words rounded-md border-t-[6px] border-t-cyan-500 shadow-lg">
        ...
        <div className="flex-auto bg-[#ffffffe5] lg:px-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 my-5 px-6 h-full w-full">
            {records.map((cuacaASH, index) => {
               var nama = new Date(cuacaASH.name).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
              ...
              var TanggalBikin = new Date(cuacaASH.createdAt).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
              ...
              var TanggalUpdate = new Date(cuacaASH.updatedAt).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
