...
const TABLE_HEAD = ["ID", "Dibuat", "Di Update", "Deskripsi", "Gambar", "Action"];
...
export function Gallery() {
  ...
  const records = galeri.slice(firstIndex, lastIndex);
  const npage = Math.ceil(galeri.length / recordsPerPage);
  ...
  // const { data } = useSWR("galeri", fetcher);
        ...
        <div className="flex-auto bg-[#ffffffe5] px-4 py-10 pt-0 lg:px-6">
          <Card className="my-5 h-full w-full overflow-scroll">
            <table className="w-full min-w-max table-auto text-left">
              ...
              <tbody>
                {records.map((galleries, index) => {
                  var TanggalBikin = new Date(galleries.createdAt).toLocaleDateString(
                    "id-ID",
                    {
                      day: "2-digit",
                      month: "long",
                  ...
                  var TanggalUpdate = new Date(galleries.updatedAt).toLocaleDateString(
                    "id-ID",
                    {
                      day: "2-digit",
                      month: "long",
