...
export function Tourism() {
  const [imagesTour, setImageTour] = useState([]);
  const [nameTour, setNameTour] = useState(new Date());
  const [fotoTour, setFotoTour] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const recordsPerPage = 20;
  const lastIndex = currentPage * recordsPerPage;
  const firstIndex = lastIndex - recordsPerPage;
  const records = imagesTour.slice(firstIndex, lastIndex);
  const npage = Math.ceil(imagesTour.length / recordsPerPage);
  ...
  const getImageTour = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/tourism"
      );
      setImageTour(response.data);
  ...
  const createCuacaHariIni = async (e) => {
    let fixDate = nameTour.toISOString();
    e.preventDefault();
    const formData = new FormData();
    formData.append("name", fixDate);
    formData.append("foto", fotoTour);
    try {
      ...
      refreshPage();
      alert("Data Prakiraan Cuaca Tourism Berhasil Ditambah");
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const deleteImageHTH = async (buletinId) => {
    await axios.delete(`http://localhost:5000/tourism/${buletinId}`);
    mutate("tourism");
    alert("Data Prakiraan Cuaca Tourism Berhasil Dihapus");
    refreshPage();
  };
                ...
                <div className="relative mb-3 w-full ">
                  <div className="flex lg:flex-wrap space-x-5">
                    <DatePicker
                      ...
                      selected={nameTour}
                      onChange={(date) => setNameTour(date)}
      ...
      <div className="bg-blueGray-100 relative mx-auto my-5 flex max-w-full flex-col break-words rounded-md border-t-[6px] border-t-cyan-500 shadow-lg">
        ...
        <div className="flex-auto bg-[#ffffffe5] lg:px-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 my-5 px-6 h-full w-full">
            {records.map((tourism, index) => {
               var nama = new Date(tourism.name).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
              ...
              var TanggalBikin = new Date(tourism.createdAt).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
              ...
              var TanggalUpdate = new Date(tourism.updatedAt).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
              ...
              return (
                ...
                >
                  ...
                  <CardFooter className="flex flex-row gap-2 self-center pt-0">
                    <a
                      href={`/dashboard/edit-tourism/${tourism.id}`}
                    >
                    ...
                    <Button
                      color="red"
                      onClick={() => deleteImageASH(tourism.id)}
