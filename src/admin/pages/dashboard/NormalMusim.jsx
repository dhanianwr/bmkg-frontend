...
export function NormalMusim() {
  ...
  const [nameNM, setNameNM] = useState(new Date());
  const [fotoNM, setFotoNM] = useState([]);
  const [currentPage, setCurrentPage] = useState(1);
  const recordsPerPage = 20;
  const lastIndex = currentPage * recordsPerPage;
  const firstIndex = lastIndex - recordsPerPage;
  const records = imagesNM.slice(firstIndex, lastIndex);
  const npage = Math.ceil(imagesNM.length / recordsPerPage);
  ...
  const getImageNM = async () => {
    try {
      const response = await axios.get("http://localhost:5000/normal-musim");
  ...
  const createCuacaHariIni = async (e) => {
    let fixDate = nameNM.toISOString();
    e.preventDefault();
    console.log(fixDate)
    ...
    try {
      await axios.post("http://localhost:5000/normal-musim", formData, {
        headers: {
          "Content-type": "multipart/form-data",
        },
      });
      refreshPage();
      alert("Data Prakiraan Normal Musim Berhasil Ditambahkan");
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const deleteImageNM = async (buletinId) => {
    await axios.delete(`http://localhost:5000/normal-musim/${buletinId}`);
    mutate("normal-musim");
    alert("Data Prakiraan Normal Musim Berhasil Dihapus");
    refreshPage();
  };
      ...
      <div className="bg-blueGray-100 relative mx-auto my-5 flex max-w-full flex-col break-words rounded-md border-t-[6px] border-t-cyan-500 shadow-lg">
        ...
        <div className="flex-auto bg-[#ffffffe5] lg:px-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 my-5 px-6 h-full w-full">
            {records.map((cuacaNM, index) => {
               var nama = new Date(cuacaNM.name).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
              ...
              var TanggalBikin = new Date(cuacaNM.createdAt).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
              ...
              var TanggalUpdate = new Date(cuacaNM.updatedAt).toLocaleDateString(
                "id-ID",
                {
                  day: "2-digit",
                  month: "long",
              ...
              return (
                ...
                >
                  ...
                  <CardFooter className="flex flex-row gap-2 self-center pt-0">
                    <a
                      href={`/dashboard/editNormalMusim/${cuacaNM.id}`}
                    >
