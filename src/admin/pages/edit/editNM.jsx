...
export function EditNM() {
  ...
  useEffect(() => {
    const getWisataNMById = async () => {
      const response = await axios.get(
        `http://localhost:5000/normal-musim/${id}`
      );
  ...
  const UpdateWisataNM = async (e) => {
    ...
    try {
      await axios.patch(`http://localhost:5000/normal-musim/${id}`, formData, {
        headers: {
          "Content-type": "multipart/form-data",
        },
      });
      navigate("/dashboard/normal-musim");
      alert("Data Prakiraan Normal Musim Berhasil Di Update")
    } catch (error) {
      console.log(error);
    }
  };
        ...
        <form onSubmit={UpdateWisataNM}>
            ...
            <div className="mr-6 flex-auto bg-[#ffffffe5] pt-10 lg:px-6">
              <div className="flex justify-end gap-4 text-center">
                <Button>
                  <Link to={`/dashboard/normal-musim`}>Cancel</Link>
                </Button>
                <Button
                  type="submit"
