...
export function EditPCH() {
  ...
  useEffect(() => {
    const getWisataPCHById = async () => {
      const response = await axios.get(
        `http://localhost:5000/prakiraan-curah-hujan/${id}`
      );
  ...
  const UpdateWisataPCH = async (e) => {
    ...
    try {
      await axios.patch(`http://localhost:5000/prakiraan-curah-hujan/${id}`, formData, {
        headers: {
          "Content-type": "multipart/form-data",
        },
      });
      navigate("/dashboard/prakiraan-sifat-hujan");
      alert("Data Prakiraan Curah Hujan Berhasil Di Update")
    } catch (error) {
      console.log(error);
    }
  };
        ...
        <form onSubmit={UpdateWisataPCH}>
            ...
            <div className="mr-6 flex-auto bg-[#ffffffe5] pt-10 lg:px-6">
              <div className="flex justify-end gap-4 text-center">
                <Button>
                  <Link to={`/dashboard/prakiraan-sifat-hujan`}>Cancel</Link>
                </Button>
                <Button
                  type="submit"
