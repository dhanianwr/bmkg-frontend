...
export function EditHTH() {
  ...
  useEffect(() => {
    const getWisataHTHById = async () => {
      const response = await axios.get(
        `http://localhost:5000/hari-tanpa-hujan/${id}`
      );
      // const fixDates = response.data.name.toISOString();
      // setName(fixDates);
  ...
  const UpdateWisataHTH = async (e) => {
    ...
    try {
      ...
      navigate("/dashboard/hari-tanpa-hujan");
      alert("Data Prakiraan Hari Tanpa Hujan Berhasil Di Update");
    } catch (error) {
      console.log(error);
    }
  };
          ...
          <form onSubmit={UpdateWisataHTH}>
            ...
            <div className="mr-6 flex-auto bg-[#ffffffe5] pt-10 lg:px-6">
              <div className="flex justify-end gap-4 text-center">
                <Button>
                  <Link to={`/dashboard/hari-tanpa-hujan`}>Cancel</Link>
                </Button>
                <Button
                  type="submit"
