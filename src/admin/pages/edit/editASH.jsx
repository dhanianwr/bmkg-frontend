...
export function EditASH() {
  ...
  useEffect(() => {
    const getWisataASHById = async () => {
      const response = await axios.get(
        `http://localhost:5000/analisis-sifat-hujan/${id}`
      );
  ...
  const UpdateWisataASH = async (e) => {
    ...
    try {
      await axios.patch(`http://localhost:5000/analisis-sifat-hujan/${id}`, formData, {
        headers: {
          "Content-type": "multipart/form-data",
        },
      });
      navigate("/dashboard/analisis-sifat-hujan");
      alert("Data Analisis Sifat Hujan Berhasil Di Update")
    } catch (error) {
      console.log(error);
    }
  };
        ...
        <form onSubmit={UpdateWisataASH}>
            ...
            <div className="mr-6 flex-auto bg-[#ffffffe5] pt-10 lg:px-6">
              <div className="flex justify-end gap-4 text-center">
                <Button>
                  <Link to={`/dashboard/analisis-sifat-hujan`}>Cancel</Link>
                </Button>
                <Button
                  type="submit"
