...
export function EditBO() {
  ...
  useEffect(() => {
    const getWisataBOById = async () => {
      const response = await axios.get(
        `http://localhost:5000/cuaca-besok/${id}`
      );
  ...
  const UpdateWisataBO = async (e) => {
    ...
    try {
      await axios.patch(`http://localhost:5000/cuaca-besok/${id}`, formData, {
        headers: {
          "Content-type": "multipart/form-data",
        },
      });
      navigate("/dashboard/cuaca-Besok");
      alert("Data Prakiraan Cuaca Besok Hari Berhasil Di Update");
    } catch (error) {
      console.log(error);
    }
  };
      ...
      <div className="bg-blueGray-100 relative mx-auto my-5 flex max-w-full flex-col break-words rounded-md border-t-[6px] border-t-cyan-500 shadow-lg">
        <div className="mb-0 rounded-t border-b-[1px] border-b-gray-300 bg-white p-2">
          <div className="flex justify-between text-center">
            <h6 className="text-blueGray-700 pl-5 text-xl">Edit Cuaca Besok</h6>
          </div>
        </div>
          ...
          <form onSubmit={UpdateWisataBO}>
            ...
            <div className="mr-6 flex-auto bg-[#ffffffe5] pt-10 lg:px-6">
              <div className="flex justify-end gap-4 text-center">
                <Button>
                  <Link to={`/dashboard/cuaca-Besok`}>Cancel</Link>
                </Button>
                <Button
                  type="submit"
